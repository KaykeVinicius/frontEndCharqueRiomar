...
export default function RelatorioPDF({ lancamentos, titulo = "RELATÓRIO DE LANÇAMENTOS" }: RelatorioPDFProps) {
  const dataAtual = new Date().toLocaleDateString("pt-BR")

  const total = lancamentos.reduce((sum, l) => {
    const valor =
      typeof l.valor === "string" ? Number.parseFloat(l.valor.replace(/[^\d,.-]/g, "").replace(",", ".")) : l.valor || 0
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.logoContainer}>
            ...
            <Text style={{ fontSize: 12, fontWeight: "bold", color: "#000000" }}>{titulo}</Text>
          </View>
        </View>
